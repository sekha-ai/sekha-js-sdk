...
export class SekhaClient {
  ...
  ): Promise<CompletionResponse & { context: Record<string, unknown> }> {
  ...
  async completeWithMemory(
  ...
  ): Promise<CompletionResponse & { searchResults: Record<string, unknown> }> {
    // Search memory
    ...
    return {
      ...completion,
      searchResults: searchResults as unknown as Record<string, unknown>,
    };
  }
